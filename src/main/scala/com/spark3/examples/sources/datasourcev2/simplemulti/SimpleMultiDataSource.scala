/*
package com.spark3.examples.sources.datasourcev2.simplemulti

import java.util

import com.spark3.examples.sources.datasourcev2.simple
...
class SimpleScan extends Scan with Batch{
  ...
  override def planInputPartitions(): Array[InputPartition] = {
    Array(new simple.SimplePartition(0,4),
      new simple.SimplePartition(5,9))
  }
...
class SimplePartitionReaderFactory extends PartitionReaderFactory {
  override def createReader(partition: InputPartition): PartitionReader[InternalRow] = new
      SimplePartitionReader(partition.asInstanceOf[simple.SimplePartition])
}


// parathion reader
class SimplePartitionReader(inputPartition: simple.SimplePartition) extends PartitionReader[InternalRow] {
